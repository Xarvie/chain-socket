...
public:
	...
	virtual int onRecv(Session *connp, int len) override {
		...
		if (connp == NULL)
			return 0;
		auto &conn = *connp;
		if (conn.userData.u32 == WS_WAIT_HANDSHAKE_WAIT) {
			...
			do {
				...
				free(responseKey);
				free(outFrame);
				conn.userData.u32 = WS_WAIT_FRAME;
				conn.readBuffer.erase(pos + 4);
				if (conn.readBuffer.size == 0)
					return 0;
			} while (0);
		} else if (conn.userData.u32 == WS_WAIT_HANDSHAKE_REQ) {
		...
		if (conn.userData.u32 == WS_WAIT_FRAME) {
			do {
				int parseSize = 0;
				for (;;) {
	...
	virtual int onConnect(Session *connp, std::string ip, int port) override {
		...
		connp->userData.u32 = WS_WAIT_HANDSHAKE_REQ;
