...
enum wsFrameType wsParseHandshake(const uint8_t *inputFrame, size_t inputLength,
    ...
    char *first = (char *) strchr((const char *) inputFrame, ' ');
    if (!first)
        return WS_ERROR_FRAME;
    first++;
...
#define strtolower(x) do { int i; for (i = 0; x[i]; i++) x[i] = tolower(x[i]); } while(0)
    ...
    while (inputPtr < endPtr && inputPtr[0] != '\r' && inputPtr[1] != '\n') {
        if (memcmp_P(inputPtr, hostField, strlen_P(hostField)) == 0) {
        ...
        } else if (memcmp_P(inputPtr, originField, strlen_P(originField)) == 0) {
            inputPtr += strlen_P(originField);
            prepare(hs->origin);
            hs->origin = getUptoLinefeed(inputPtr);
        } else if (memcmp_P(inputPtr, protocolField, strlen_P(protocolField)) == 0) {
            inputPtr += strlen_P(protocolField);
            subprotocolFlag = TRUE;
        } else if (memcmp_P(inputPtr, keyField, strlen_P(keyField)) == 0) {
            inputPtr += strlen_P(keyField);
            prepare(hs->key);
            hs->key = getUptoLinefeed(inputPtr);
        } else if (memcmp_P(inputPtr, versionField, strlen_P(versionField)) == 0) {
            ...
            free(versionString);
        } else if (memcmp_P(inputPtr, connectionField, strlen_P(connectionField)) == 0) {
        ...
        } else if (memcmp_P(inputPtr, upgradeField, strlen_P(upgradeField)) == 0) {
